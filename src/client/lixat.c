...
struct thread_parameters_s {
    ...
    double          *close;
    double          *total; /* open + begin + commit/rollback + close */
};
...
int exec_benchmark(void)
{
    ...
    for (i=0; i<clients; ++i) {
        ...
        parameters[i].total = total_array + i*bench_cycles;
    ...
    for (i=0; i<clients; ++i)
        for (j=0; j<bench_cycles; ++j) {
            ...
            total_array[i*bench_cycles+j] = 0.0;
        }
...
gpointer perform_benchmark(gpointer data)
{
    ...
    for (c=0; c<tp->cycles; ++c) {
        ...
        if (c == tp->cycles-1 || open_close) {
            ...
            tp->close[c] = 0.0;
        tp->total[c] = tp->open[c] + tp->begin[c] + tp->comrol[c] +
            tp->close[c];
