                         ...
                         struct lixa_msg_s *msg)
{
    ...
    TRY {
        ...
        if (xmlStrcmp(cur->name, LIXA_XML_MSG_TAG_MSG)) THROW(
            ROOT_TAG_IS_NOT_MSG);
        /* check there is at least a child */
        if (NULL == cur->xmlChildrenNode) THROW(MISSING_TAGS);
        /* reset output message */
        memset(msg, 0, sizeof(struct lixa_msg_s));
        /* retrieve verb */
        if (NULL == (tmp = xmlGetProp(cur, LIXA_XML_MSG_PROP_VERB))) THROW(
            VERB_NOT_FOUND);
        msg->header.pvs.verb = (int) strtol((char *) tmp, NULL, 0);
        xmlFree(tmp);
        /* retrieve step */
        if (NULL == (tmp = xmlGetProp(cur, LIXA_XML_MSG_PROP_STEP))) THROW(
            STEP_NOT_FOUND);
        msg->header.pvs.step = (int) strtol((char *) tmp, NULL, 0);
        xmlFree(tmp);
        /* retrieve level */
        if (msg->header.pvs.step == LIXA_MSG_STEP_INCR) {
            if (NULL == (tmp = xmlGetProp(cur, LIXA_XML_MSG_PROP_LEVEL))) THROW(
                LEVEL_NOT_FOUND);
        ...
        if (LIXA_RC_OK != ret_cod) THROW(CHILD_LEVEL_ERROR);
...
int lixa_msg_deserialize_close_8(xmlNodePtr cur, struct lixa_msg_s *msg)
{
    enum Exception
    {
        RMID_NOT_FOUND, XML_UNRECOGNIZED_TAG, NONE
    } excp;
    int ret_cod = LIXA_RC_INTERNAL_ERROR;
                                        ...
                                        struct lixa_msg_body_answer_s *answer)
{
    enum Exception
    {
        RC_NOT_FOUND, XML_UNRECOGNIZED_TAG, NONE
    } excp;
    int ret_cod = LIXA_RC_INTERNAL_ERROR;
...
int lixa_msg_deserialize_qrcvr_8(xmlNodePtr cur, struct lixa_msg_s *msg)
{
    enum Exception
    {
        JOB_NOT_FOUND, CONFIG_DIGEST_NOT_FOUND, XML_UNRECOGNIZED_TAG, NONE
    } excp;
    int ret_cod = LIXA_RC_INTERNAL_ERROR;
...
int lixa_msg_deserialize_qrcvr_16(xmlNodePtr cur, struct lixa_msg_s *msg)
{
    ...
    TRY {
        while (NULL != cur) {
                ...
                while (NULL != cur2) {
                    if (!xmlStrcmp(
                        ...
                        if (!lixa_xid_deserialize(
                            ...
                            xmlFree(tmp);
                    } else THROW(XML_UNRECOGNIZED_TAG1);
                    cur2 = cur2->next;
                } /* while (NULL != child) */
            } else if (!xmlStrcmp(cur->name, LIXA_XML_MSG_TAG_RSRMGRS)) {
                xmlNodePtr cur2 = cur->xmlChildrenNode;
            ...
            } else THROW(XML_UNRECOGNIZED_TAG2);
            cur = cur->next;
        } /* while (NULL != cur) */
...
int lixa_msg_deserialize_reg_8(xmlNodePtr cur, struct lixa_msg_s *msg)
{
    ...
    TRY {
        ...
        if (!xmlStrcmp(cur->name, LIXA_XML_MSG_TAG_AX_REG_EXEC)) {
            /* retrieve control thread properties */
            if (NULL == (tmp = xmlGetProp(cur, LIXA_XML_MSG_PROP_RMID))) THROW(
                RMID_NOT_FOUND);
            msg->body.reg_8.ax_reg_exec.rmid =
                (int) strtol((char *) tmp, NULL, 0);
            xmlFree(tmp);
            if (NULL == (tmp = xmlGetProp(cur, LIXA_XML_MSG_PROP_FLAGS))) THROW(
                FLAGS_NOT_FOUND);
            msg->body.reg_8.ax_reg_exec.flags =
                (int) strtol((char *) tmp, NULL, 0);
            xmlFree(tmp);
            if (NULL == (tmp = xmlGetProp(cur, LIXA_XML_MSG_PROP_RC))) THROW(
                RC_NOT_FOUND);
            ...
            if (!xmlStrcmp(cur->name, LIXA_XML_MSG_TAG_ANSWER)) {
                ...
                while (NULL != cur2) {
                    if (!xmlStrcmp(cur2->name, LIXA_XML_MSG_TAG_TRAN)) {
                        ...
                        xmlFree(tmp);
                        XID xid;
                        if (!lixa_xid_deserialize(&xid, tran.xid)) THROW(
                            XID_DESERIALIZE_ERROR);
